...
レポートの状態は SQLite（`better-sqlite3`）で `data/reports.db` に永続化しています。テーブルはアプリ起動時に自動作成されるため、マイグレーション作業は不要です。スレッド ID（Discord のグローバルにユニークな ID）を主キーにしているので、複数サーバーで運用してもデータが混ざりません。`data/*.db*` は `.gitignore` 済みでリポジトリには含まれません。
